                        ...
                        <div class="product-info">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="mb-1 product-price itemprice">
                                    ...
                                    <span class="er-each">${ (item.available_weights[0] === "each") ? '/each' : (item.available_weights[0] === "gram") ? '/1G' : (item.available_weights[0] === "eighth ounce") ? '/3.5G' : (item.available_weights[0] === "quarter ounce") ? '/7G' : (item.available_weights[0] === "half ounce") ? '/14G' : (item.available_weights[0] === "ounce") ? '/28G' : (item.available_weights[0] === "half gram") ? '/0.5G' : '00.00'   }</span>
                                </div>
                            </div>
                            <div class="product-action mt-2">
                                <div class="d-grid gap-2">
                                    <a href="/views/product-details.html?objectID=${item.objectID}" class="btn btn-dark btn-ecomm">Product Details</a>
                                    <a href="/views/cart.html" class="btn btn-light btn-ecomm"><i class="bx bxs-cart-add"></i>add to cart</a>
                                </div>
                            </div>
                        </div>
                    </div>
